...
def getAcountMatrix(p,type):
    ...
    for seq in chunk_sequences:
        ...
        for i in range(1,71):
            ...
            row_list.append(binSeq.count("A") if type == "m6A" else binSeq.count("C"))
...
if __name__ == "__main__":
    ...
    for i in ss:
        res.append(pool.apply_async(func=getAcountMatrix, args=(i,"m4C"))) # asynchronization
    aa = zip(*(res[i].get().tolist() for i in ss))
    with open("/home/wanghm/wanghm_R/methylation/m4C_ges", "w+") as f:
        for i in aa:
            print(i)
            total = sum(i)
            f.writelines(str(total)+"\n")
    # result = list(map(lambda x:sum(x), aa))
    # print(result)
    # print(len(result))
    #
    # with open("/home/wanghm/wanghm_R/methylation/m4C_ge2", "w+") as f:
    #     for i in result:
    #         f.writelines(str(i)+"\n")
